...
export default class FoodDetailComponent extends Component {
    ...
    displayPrice = () => {
        if (this.state.selectedFood.price) {
            return (<Text style={{fontSize: 30}}>·{this.state.selectedFood.price}</Text>);
        }
    };
    ...
    handleTimeStr = (isOpen) => {
        ...
        if (isOpen) {
            return this.formatTime(this.state.selectedFood.hours[0].open[weekDayEnum].end);
        } else {
            return this.formatTime(this.state.selectedFood.hours[0].open[weekDayEnum].start);
        }
    };
                ...
                <View>
                    {this.state.selectedFood.hours[0].open.map((day) => (
                        <View key={day.day} style={{flexDirection: 'row'}}>
                            ...
                            <Text>{this.formatTime(day.start)} - {this.formatTime(day.end)}</Text>
                        </View>))
                    }
                </View>
            )
        ...
        return (
            <View style={{flex: 1, backgroundColor: '#dcdcdc'}}>
                <View style={[styles.card, styles.informationCard]}>
                    <View style={{flex: 1}}>
                        <Image source={{ uri: this.state.selectedFood.image}} style={styles.foodImage}/>
                    </View>
                    <View style={{flex: 0.5, alignItems: 'center'}}>
                        <Text style={{fontSize: 25}}>{this.state.selectedFood.name}</Text>
                        <View style={{flexDirection: 'row', alignItems: 'center'}}>
                            <StarRatingComponent rating={this.state.selectedFood.rating} size={24}/>
                            {this.displayPrice()}
                        </View>
                        <Text>{this.state.selectedFood.display_phone}</Text>
                        <Text>
                            <Text>
                                {this.state.selectedFood.location.address1}, {this.state.selectedFood.location.city},{' '}
                ...
                <View style={[styles.card, styles.actionCard]}>
                    <TouchableButtonComponent size={36} text={'Directions'} iconName={'directions'} action={this.openMap.bind(this)}/>
                    <TouchableButtonComponent size={36} text={'Call'} iconName={'call'} action={this.call.bind(this)}/>
                    <TouchableButtonComponent size={36} text={'Yelp'} iconName={'public'} action={this.openYelp.bind(this)}/>
                </View>
                {this.hasHours()}
            </View>
        );
